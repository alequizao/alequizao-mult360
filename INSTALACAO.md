...
### 11. Respostas Automáticas sem fila + usuário sem e-mail + GitHub (2026-09-09, 13h)
- **Menu → Respostas Automáticas** (`/auto-replies`): nome, palavras-chave (separadas por `;` ou vírgula), modo de comparação (contém / palavra inteira / começa com / exata / regex), resposta (aceita `{{name}}`, `{{firstName}}`), intervalo anti-repetição por contato (min), "só sem atendente", "não acionar saudação/chatbot depois de responder", ativa/inativa, contador de disparos e caixa de teste. Backend: tabela `AutoReplies`, `AutoReplyService.ts`, rotas `/auto-replies`, hook no `wbotMessageListener.ts` logo após `verifyMessage` (só mensagens recebidas, fora de grupo). Regra de exemplo já cadastrada: **Areia**. Comparação ignora maiúsculas e acentos e procura em qualquer posição da frase ("Gostaria de comprar areia" casa).
- **Usuários com ou sem e-mail:** o campo aceita e-mail válido OU usuário simples (`letras, números, . _ -`, 3–40). Validação nos services de usuário (backend) e no modal (frontend); rótulo "E-mail ou usuário" no login e no cadastro.
- **Logo com fundo transparente** (flood-fill do branco): `frontend/public/logo.png`, `src/assets/logo.png`, ícones do PWA e `backend/public/logotipos/*.png` regenerados.
- **GitHub:** `https://github.com/alequizao/<senha>-mult360` (privado) — sem `.env`, `node_modules`, `dist/build`, `backend/public` (mídias) e `backend/private`; `.env.example` no lugar.
- **Kanban:** as colunas são as Etiquetas marcadas como "Kanban" (Etiquetas → nova → ligar a chave Kanban). Sem etiquetas Kanban só aparece "Em aberto".

### 12. Permissões e login (2026-09-09, 13h40)
- **Todos os atendimentos para todos:** `ListTicketsService.ts` sem filtro por atendente/fila (usuários e administradores veem novos e antigos, inclusive Resolvidos); no frontend, `TicketsListCustom` sem filtro por fila/perfil e "Todos" ligado por padrão.
- **Login por seleção:** rota pública `GET /auth/users-list` (nome + login, sem senha) e a tela de login mostra um seletor de usuário; só a senha é digitada. Lembra o último usuário escolhido no aparelho.
