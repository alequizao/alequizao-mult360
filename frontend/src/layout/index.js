...
const useStyles = makeStyles((theme) => ({
  root: {
    ...
    '& .MuiButton-outlinedPrimary': {
	  ...
	  backgroundColor: theme.mode === 'light' ? '#2563EB' : '#1c1c1c',
      //border: theme.mode === 'light' ? '1px solid rgba(0 124 102)' : '1px solid rgba(255, 255, 255, 0.5)',
    },
    '& .MuiTab-textColorPrimary.Mui-selected': {
      color: theme.mode === 'light' ? '#2563EB' : '#FFF',
    }
  },
  avatar: {
    width: "100%",
...
const LoggedInLayout = ({ children, themeToggle }) => {
  ...
  const [volume, setVolume] = useState(localStorage.getItem("volume") ? Number(localStorage.getItem("volume")) : 1);
  ...
  useEffect(() => {
    ...
    // ALEQUIZAO: bloqueio "um dispositivo por vez" removido — o mesmo usuário pode ficar logado
    // em vários aparelhos ao mesmo tempo (antes, um login novo limpava o storage dos outros).
