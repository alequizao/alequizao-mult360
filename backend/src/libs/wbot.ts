import { fixJid, setLidPn } from "./lidCache";
    ...
    try {
      (async () => {
        ...
        wsocket = makeWASocket({
          ...
          browser: [
            process.env.BROWSER_CLIENT || "ALEQUIZÃO MULT360",
        ...
        // ALEQUIZAO: suporte a LID — contatos salvos com o id interno (14+ dígitos) são enviados para @lid
        {
          const origSend = wsocket.sendMessage.bind(wsocket);
          wsocket.sendMessage = ((jid: any, ...rest: any[]) => origSend(fixJid(jid), ...rest)) as any;
          const origPresence = wsocket.sendPresenceUpdate.bind(wsocket);
          wsocket.sendPresenceUpdate = ((type: any, jid?: any) => origPresence(type, fixJid(jid))) as any;
          const origPic = wsocket.profilePictureUrl.bind(wsocket);
          wsocket.profilePictureUrl = ((jid: any, ...rest: any[]) => origPic(fixJid(jid), ...rest)) as any;
          const origRead = wsocket.readMessages.bind(wsocket);
          wsocket.readMessages = ((keys: any[]) => origRead((keys || []).map((k: any) => ({ ...k, remoteJid: fixJid(k.remoteJid) })))) as any;
          wsocket.ev.on("chats.phoneNumberShare" as any, (d: any) => { if (d?.lid && d?.jid) setLidPn(d.lid, d.jid); });
        }
