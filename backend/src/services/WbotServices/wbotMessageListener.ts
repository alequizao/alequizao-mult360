import { setLidPn, getPnForLid, isLidNumber } from "../../libs/lidCache";
...
import { encontrarResposta } from "../AutoReplyService/AutoReplyService";
...
const getSenderMessage = (
  ...
  return senderId && jidNormalizedUser(resolveLid(senderId, msg));
};

// ALEQUIZAO: converte jid @lid no telefone real (senderPn/participantPn vêm da Baileys; cache cobre o resto)
const resolveLid = (jid: string, msg: proto.IWebMessageInfo): string => {
  if (!jid || !jid.endsWith("@lid")) return jid;
  const k: any = msg.key || {};
  const pn = k.senderPn || k.remoteJidAlt || k.participantPn || k.participantAlt || getPnForLid(jid);
  if (pn) { setLidPn(jid, pn); return jidNormalizedUser(pn); }
  return jid;
};

const getContactMessage = async (msg: proto.IWebMessageInfo, wbot: Session) => {
  const isGroup = msg.key.remoteJid.includes("g.us");
  const chatJid = resolveLid(msg.key.remoteJid, msg);
  const rawNumber = chatJid.replace(/\D/g, "");
  const lid = msg.key.remoteJid.endsWith("@lid") && chatJid !== msg.key.remoteJid ? msg.key.remoteJid : undefined;
  return isGroup
    ? {
...
const verifyContact = async (
  ...
  // ALEQUIZAO: contato que foi salvo com o LID passa a usar o telefone real
  const lidDigits = (msgContact as any).lid ? String((msgContact as any).lid).replace(/\D/g, "") : "";
  if (lidDigits && isLidNumber(lidDigits)) {
    try {
      const antigo = await Contact.findOne({ where: { number: lidDigits, companyId } });
      const certo = await Contact.findOne({ where: { number: msgContact.id.replace(/\D/g, ""), companyId } });
      if (antigo && !certo) await antigo.update({ number: msgContact.id.replace(/\D/g, "") });
    } catch (e) { logger.warn(`LID merge falhou: ${e}`); }
  }
...
const handleMessage = async (
  ...
  try {
    ...
    if (!msg.key.fromMe && !isGroup && bodyMessage) {
      try {
        const regra = await encontrarResposta(companyId, bodyMessage, contact.id, !!ticket.userId);
        if (regra) {
          const enviada = await wbot.sendMessage(`${contact.number}@s.whatsapp.net`, { text: formatBody(regra.reply, contact) });
          await verifyMessage(enviada, ticket, contact);
          logger.info(`Resposta automática "${regra.name}" enviada para ${contact.number}`);
          if (regra.stopFlow) return;
        }
      } catch (e) {
        logger.error(`Resposta automática falhou: ${e}`);
      }
    }
...
// ALEQUIZAO: salva automaticamente contatos da agenda/histórico do WhatsApp com nome e foto
const sincronizarContatos = async (wbot: Session, companyId: number, lista: any[], origem: string) => {
  let n = 0;
  for (const c of lista || []) {
    try {
      const id: string = c?.id || "";
      if (c?.lid && id.endsWith("@s.whatsapp.net")) setLidPn(c.lid, id);
      let jid = id;
      if (jid.endsWith("@lid")) { const pn = getPnForLid(jid); if (!pn) continue; jid = `${pn}@s.whatsapp.net`; }
      if (!jid.endsWith("@s.whatsapp.net")) continue;
      const number = jid.replace(/\D/g, "");
      if (!number || number.length < 8) continue;
      const name: string = c.name || c.notify || c.verifiedName || "";
      const existente = await Contact.findOne({ where: { number, companyId } });
      if (existente && (!name || (existente.name && !/^\d+$/.test(existente.name)))) {
        if (c.imgUrl === "changed") {
          try { const pic = await wbot.profilePictureUrl(jid, "image"); if (pic) await existente.update({ profilePicUrl: pic }); } catch (e) { /* sem foto */ }
        }
        continue;
      }
      let profilePicUrl = "";
      try { profilePicUrl = (await wbot.profilePictureUrl(jid, "image")) || ""; } catch (e) { profilePicUrl = ""; }
      await CreateOrUpdateContactService({ name: name || number, number, isGroup: false, profilePicUrl, companyId, whatsappId: wbot.id });
      n++;
    } catch (e) { logger.warn(`sync contato (${origem}) falhou: ${e}`); }
  }
  if (n) logger.info(`Contatos sincronizados (${origem}): ${n}`);
};

const wbotMessageListener = async (wbot: Session, companyId: number): Promise<void> => {
  try {
    wbot.ev.on("contacts.upsert" as any, (contacts: any[]) => sincronizarContatos(wbot, companyId, contacts, "contacts.upsert"));
    wbot.ev.on("contacts.update" as any, (contacts: any[]) => sincronizarContatos(wbot, companyId, contacts, "contacts.update"));
    wbot.ev.on("messaging-history.set" as any, (h: any) => sincronizarContatos(wbot, companyId, h?.contacts || [], "history"));
