...
const UpdateUserService = async ({
  ...
  const schema = Yup.object().shape({
    ...
    email: Yup.string().test("email-ou-usuario", "Informe um e-mail válido ou um nome de usuário (letras, números, ponto, hífen ou _)", v => !!v && (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v) || /^[a-z0-9._-]{3,40}$/i.test(v))),
    profile: Yup.string(),
    password: Yup.string(),
